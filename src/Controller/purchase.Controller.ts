...
import { checkInventoryAndUpdate } from "../repositories/purchases.Repo";
import { createPayment } from "../repositories/payment.Repo";
import { Role } from "../utils/enums/Roles";
export const pusrchaseItems = async (req: Request | any, res: Response) => {
  console.log("------controller:pusrchaseItems---------");

  try {
    if (req.roleName !== Role.buyer) {
