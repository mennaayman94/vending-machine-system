...
export const salesReportsController = async (req: Request|any, res: Response) => {
    ...
    try {
      ...
      res.status(200).json({salesQuantity:report.totalSalesQuantity,totalSales:report.totalSales});
    } catch (error) {
      res.status(400).json({ error: "Something went wrong in getting reports", message: error });
    }
  };