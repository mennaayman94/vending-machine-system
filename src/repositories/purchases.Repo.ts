    ...
    const transaction = await prisma.$transaction(async (tx: any) => {
      const transactionsItems = await tx.item.findMany({
        where: {
          id: {
            in: items.map(
              (item: { itemId: string; quantity: number }) => item.itemId
            ),
          },
        },
      });
      ...
      if (totalSum > cash) {
        ...
        for (let i = 0; i < transactionsItems.length; i++) {
          ...
          if (itemsHashMap[transactionsItems[i].id]) {
            ...
            totalCost =
              itemsHashMap[transactionsItems[i].id] *
              transactionsItems[i].price;

            await createPurchase({
              createdAt: currentDate.toISOString(),
              categoryId: transactionsItems[i].categoryId,
              totalCost,
              quantity: itemsHashMap[transactionsItems[i].id],
              itemId: transactionsItems[i].id,
              paymentId: paymentId,
            });
          }
        }
